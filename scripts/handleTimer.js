...
function mainLoop() {
  ...
  timer.addEventListener("secondsUpdated", function (e) {
    ...
    $(".timer_area").html(timer.getTimeValues().toString());
    if (
  ...
  if (
    ...
    document.querySelector(".pop-body").innerHTML = `
    
        You already have a session of <b>${minTwoDigits(
          localStorage.getItem("hr")
        )}:${minTwoDigits(localStorage.getItem("min"))}:${minTwoDigits(
      localStorage.getItem("sec")
    )} </b> paused in your system. Do you wanna
              continue the session or start over. Click following buttons to choose
              your action. <br />
              <b
                >Remember, You will never be able to retrive the lost session in
                your lifetime 🤨
              </b>
